...
import Hero from '@/app/components/Common/Heroes/Whitepaper';
import FormContainer from '@/app/components/Common/Form/components/FormContainer';
import Form from '@/app/components/Common/Form/Whitepaper';
...
export async function generateMetadata(__: unknown, parent: ResolvingMetadata): Promise<Metadata> {
  const pageMetadata = await getPageMetadata('whitepaper');
  const parentMetadata = await parent;
  return parseMetadata({ pageMetadata, parentMetadata });
}

const WhitepaperPage = async () => {
  const strapiData = await getStrapiData('whitepaper');
