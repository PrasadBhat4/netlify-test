...
import Hero from '@/app/components/Common/Heroes/Contact/Sales';
import FormContainer from '@/app/components/Common/Form/components/FormContainer';
import Form from '@/app/components/Common/Form/ContactUs';
...
const SalesPage = async () => {
  const strapiData = await getStrapiData('contact');
  ...
  return (
    <Container className='flex relative flex-col gap-16 my-16 lg:flex-row md:gap-12 lg:gap-6 md:max-lg:my-12 lg:mt-20 xl:mt-38'>
      {hero && <Hero data={hero} className='w-full lg:w-1/2 flex-1 lg:basis-1/2 max-w-none' />}
      {form && (
        <FormContainer className='w-full lg:w-1/2 flex-1 lg:basis-1/2 max-w-none'>
