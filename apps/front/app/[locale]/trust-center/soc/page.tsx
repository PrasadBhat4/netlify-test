...
import Form from '@/app/components/Common/Form/SOC';
...
export async function generateMetadata(__: unknown, parent: ResolvingMetadata): Promise<Metadata> {
  const pageMetadata = await getPageMetadata('trust-center-soc');
  const parentMetadata = await parent;
  return parseMetadata({ pageMetadata, parentMetadata });
}

const TrustCenterSOCPage = async () => {
  const strapiData = await getStrapiData('trust-center-soc');
...
export default TrustCenterSOCPage;
