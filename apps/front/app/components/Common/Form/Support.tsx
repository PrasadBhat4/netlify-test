'use client';

/* eslint-disable no-underscore-dangle */
/* eslint-disable jsx-a11y/control-has-associated-label */
import { useState } from 'react';
import { z } from 'zod';
import SubmitButton from '@/app/components/Common/Form/components/SubmitButton';
import TextField from '@/app/components/Common/Form/components/TextField';
import Success from '@/app/components/Common/Form/Success';
import { HandleChangeType, StrapiFormSupport, StrapiSuccess } from '@/app/lib/types';
import SelectField from '@/app/components/Common/Form/components/SelectField';

import { GDPRConsentText } from '../GDPRConsentText';

interface Props {
  data: StrapiFormSupport;
  success: StrapiSuccess;
}

const PLANS = [
  { id: 1, Label: 'Free Open Source', Value: 'free_open_source' },
  { id: 2, Label: 'Free Trial', Value: 'free_trial' },
  { id: 3, Label: 'Lite', Value: 'lite' },
  { id: 4, Label: 'Pro', Value: 'pro' },
  { id: 5, Label: 'Enterprise', Value: 'enterprise' },
];

const ISSUE_TYPES = [
  { id: 1, Label: 'Feature Requests', Value: 'feature' },
  { id: 2, Label: 'Support Request - Billing', Value: 'billing' },
  { id: 3, Label: 'Support Request - Not A blocker', Value: 'non_blocker' },
  { id: 4, Label: 'Support Request - Blockers', Value: 'blocker' },
];

const LABELS = {
  PLANS: 'CodeRabbit Plan',
  ISSUE_TYPES: 'Issue Type',
  GITHUB_URL: 'Git Issue/PR URL',
};

const PLACEHOLDERS = {
  PLANS: 'Select a plan',
  ISSUE_TYPES: 'Select an issue type',
  GITHUB_URL: 'EX: https://github.com/PROJECTNAME/REPONAME/pull/123',
};

const Form = ({ data, success }: Readonly<Props>) => {
  const schema = z.object({
    '0-1/firstname':
      data?.FirstName?.isRequired === true
        ? z
            .string({
              required_error: 'This field is required',
              invalid_type_error: 'This field must be a string',
            })
            .min(2, { message: 'This field is required' })
            .trim()
        : z.string().optional(),
    '0-1/lastname':
      data?.LastName?.isRequired === true
        ? z
            .string({
              required_error: 'This field is required',
              invalid_type_error: 'This field must be a string',
            })
            .min(2, { message: 'This field is required' })
            .toLowerCase()
            .trim()
        : z.string().optional(),
    '0-2/name':
      data?.CompanyName?.isRequired === true
        ? z
            .string({
              required_error: 'This field is required',
              invalid_type_error: 'This field must be a string',
            })
            .min(2, { message: 'This field is required' })
            .toLowerCase()
            .trim()
        : z.string().optional(),
    '0-1/email':
      data?.WorkEmail?.isRequired === true
        ? z
            .string({
              required_error: 'This field is required',
              invalid_type_error: 'This field must be a string',
            })
            .min(2, { message: 'This field is required' })
            .email({ message: 'Invalid email address' })
            .toLowerCase()
            .trim()
        : z.string().optional(),
    '0-5/ticket_problem_url': z
      .string({
        invalid_type_error: 'This field must be a string',
      })
      .trim()
      .optional(),
    '0-5/content': z
      .string({
        required_error: 'This field is required',
        invalid_type_error: 'This field must be a string',
      })
      .min(1, { message: 'This field is required' })
      .toLowerCase()
      .trim(),
    '0-5/coderabbit_plan': z
      .string({
        required_error: 'This field is required',
        invalid_type_error: 'This field must be a string',
      })
      .min(1, { message: 'This field is required' })
      .trim(),
    '0-5/issue_type': z
      .string({
        required_error: 'This field is required',
        invalid_type_error: 'This field must be a string',
      })
      .min(1, { message: 'This field is required' })
      .trim(),
  });
  const [formValues, setFormValues] = useState<z.infer<typeof schema>>({
    '0-1/firstname': '',
    '0-1/lastname': '',
    '0-2/name': '',
    '0-1/email': '',
    '0-5/coderabbit_plan': '',
    '0-5/issue_type': '',
    '0-5/ticket_problem_url': '',
    '0-5/content': '',
  });
  const [errors, setErrors] = useState<{
    [key: string]: string;
  }>({});
  const [isFormSent, setIsFormSent] = useState(false);

  const handleChange = (value: HandleChangeType) => {
    setFormValues({ ...formValues, ...value });
  };

  const submitHandler = async (e: React.SyntheticEvent) => {
    e.stopPropagation();
    e.preventDefault();

    setErrors({});

    const validationSchema = schema.safeParse(formValues);
    if (!validationSchema.success && validationSchema.error.issues.length) {
      const issues = validationSchema.error.issues.reduce((acc: { [key: string]: string }, current) => {
        const path = current.path.join('/');
        acc[path] = current.message;
        return acc;
      }, {});

      setErrors(issues);
      return;
    }

    // send data
    const portalId = process.env.NEXT_PUBLIC_HUBSPOT_PORTAL_ID ?? '';
    const formGuid = process.env.NEXT_PUBLIC_HUBSPOT_CONTACT_SUPPORT_FORM_GUID ?? '';
    const url = `https://api.hsforms.com/submissions/v3/integration/submit/${portalId}/${formGuid}`;

    // Get HubSpot tracking cookie
    const hubspotutk =
      document.cookie
        .split(';')
        .find(c => c.trim().startsWith('hubspotutk='))
        ?.split('=')[1] || '';

    const mappedFields = Object.entries(validationSchema.data as Record<string, string>).map(([key, value]) => {
      const [objectTypeId, name] = key.split('/');
      return {
        name,
        value,
        objectTypeId,
      };
    });

    const composedFields = [
      {
        name: 'hs_ticket_priority',
        value: 'MEDIUM',
        objectTypeId: '0-5',
      },
      {
        name: 'subject',
        value: `${validationSchema.data?.['0-1/email']} - ${validationSchema.data?.['0-2/name']}`,
        objectTypeId: '0-5',
      },
    ];

    const formDataToSend = {
      submittedAt: `${new Date().getTime()}`,
      fields: [...mappedFields, ...composedFields],
      context: {
        hutk: hubspotutk,
        pageUri: window?.location?.href ?? 'https://coderabbit.ai/contact-us/support',
        pageName: 'Contact Us Support',
      },
    };

    try {
      await fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(formDataToSend),
      });
      setIsFormSent(true);
      // eslint-disable-next-line no-restricted-globals
      scroll({ top: 0, behavior: 'smooth' });
    } catch (error) {
      throw new Error('An error occurred while submitting the form');
    }
    setIsFormSent(true);
  };

  return !isFormSent ? (
    <form onSubmit={submitHandler}>
      <p className='font-semibold text-body-md-sm lg:text-body-md'>{data.Title}</p>
      <div className='flex flex-wrap w-full'>
        <div className='flex flex-col w-full gap-6 mt-8 md:flex-row md:justify-between md:items-start'>
          <TextField
            label={data?.FirstName?.Label}
            placeholder={data?.FirstName?.Placeholder}
            isEmail={data?.FirstName?.isEmail}
            isTextarea={data?.FirstName?.isTextarea}
            isRequired={data?.FirstName?.isRequired}
            hasError={!!errors['0-1/firstname']}
            message={errors['0-1/firstname']}
            name='0-1/firstname'
            handleChange={handleChange}
            className={`w-full  ${errors.firstname ? 'mt-9' : 'mt-3'}`}
          />
          <TextField
            label={data?.LastName?.Label}
            placeholder={data?.LastName?.Placeholder}
            isEmail={data?.LastName?.isEmail}
            isTextarea={data?.LastName?.isTextarea}
            isRequired={data?.LastName?.isRequired}
            hasError={!!errors['0-1/lastname']}
            message={errors['0-1/lastname']}
            name='0-1/lastname'
            handleChange={handleChange}
            className={`w-full  ${errors.lastname ? 'mt-9' : 'mt-3'}`}
          />
        </div>
        <TextField
          label={data?.CompanyName?.Label}
          placeholder={data?.CompanyName?.Placeholder}
          isEmail={data?.CompanyName?.isEmail}
          isTextarea={data?.CompanyName?.isTextarea}
          isRequired={data?.CompanyName?.isRequired}
          hasError={!!errors['0-2/name']}
          message={errors['0-2/name']}
          name='0-2/name'
          handleChange={handleChange}
          className='w-full mt-8'
        />
        <TextField
          label={data?.WorkEmail?.Label}
          placeholder={data?.WorkEmail?.Placeholder}
          isEmail={data?.WorkEmail?.isEmail}
          isTextarea={data?.WorkEmail?.isTextarea}
          isRequired={data?.WorkEmail?.isRequired}
          hasError={!!errors['0-1/email']}
          message={errors['0-1/email']}
          name='0-1/email'
          handleChange={handleChange}
          className='w-full mt-8'
        />
        <SelectField
          label={LABELS.PLANS}
          placeholder={PLACEHOLDERS.PLANS}
          options={PLANS}
          isRequired
          hasError={!!errors['0-5/coderabbit_plan']}
          message={errors['0-5/coderabbit_plan']}
          name='0-5/coderabbit_plan'
          handleChange={handleChange}
          className='w-full mt-8'
        />
        <SelectField
          label={LABELS.ISSUE_TYPES}
          placeholder={PLACEHOLDERS.ISSUE_TYPES}
          options={ISSUE_TYPES}
          isRequired
          hasError={!!errors['0-5/issue_type']}
          message={errors['0-5/issue_type']}
          name='0-5/issue_type'
          handleChange={handleChange}
          className='w-full mt-8'
        />
        <TextField
          label={LABELS.GITHUB_URL}
          placeholder={PLACEHOLDERS.GITHUB_URL}
          isEmail={false}
          isTextarea={false}
          isRequired={false}
          hasError={!!errors['0-5/ticket_problem_url']}
          message={errors['0-5/ticket_problem_url']}
          name='0-5/ticket_problem_url'
          handleChange={handleChange}
          className='w-full mt-8'
        />
        <TextField
          label={data?.HowCanWeHelp?.Label}
          placeholder={data?.HowCanWeHelp?.Placeholder}
          isEmail={data?.HowCanWeHelp?.isEmail}
          isTextarea={data?.HowCanWeHelp?.isTextarea}
          isRequired={data?.HowCanWeHelp?.isRequired}
          hasError={!!errors['0-5/content']}
          message={errors['0-5/content']}
          name='0-5/content'
          handleChange={handleChange}
          className='w-full mt-8'
        />
      </div>
      <div className='flex items-center justify-between'>
        <GDPRConsentText />
        <SubmitButton text={data.Button.Text} submitHandler={submitHandler} />
      </div>
    </form>
  ) : (
    <Success success={success} />
  );
};

export default Form;
