...
const Form = ({ data, className = '', inputClassName = '' }: Readonly<Props>) => {
  ...
  const submitHandler = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      setIsFormSent(true);
      setSuccessMessage({
        '0-1/email': `You've subscribed to CodeRabbit's Newsletter!`,
      });
    ...
    <div className={tm('flex flex-col w-full md:w-[21.25rem] lg:w-[27rem] mt-6 md:mt-0', className)}>
      <form onSubmit={submitHandler} className='relative'>
        <TextField
          ...
          className={tm('w-full mt-8 ', inputClassName)}
          arrowPosition='right'
          arrowSubmits
          resetField={resetField}
        />
        <GDPRConsentText type='signup' className='dark:text-white text-neutral-900' />
      </form>
    </div>
  );
};
