...
const Form = ({ data, success }: Readonly<Props>) => {
  ...
  const submitHandler = async (e: React.SyntheticEvent) => {
    ...
    setErrors({});
    const validationSchema = await schema.safeParseAsync(formValues);
    ...
    const formGuid = process.env.NEXT_PUBLIC_HUBSPOT_WHITEPAPER_FORM_GUID ?? '';
    ...
    const formDataToSend = {
      ...
      fields: mappedFields,
      context: {
        pageUri: window?.location?.href ?? 'https://coderabbit.ai/whitepaper',
        pageName: 'Whitepaper',
      },
    };
  ...
  return !isFormSent ? (
    <form onSubmit={submitHandler}>
      <h3 className='font-semibold text-body-md-sm lg:text-body-md'>{data.Title}</h3>
