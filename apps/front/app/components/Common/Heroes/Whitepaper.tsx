...
const Hero = ({ data = {} }: Readonly<Props>) => (
  <div className='flex flex-col md:max-lg:justify-between md:max-lg:gap-x-12 px-8 py-6 md:py-16 md:px-14 lg:p-20 w-full lg:max-w-[34.125rem] xl:max-w-[51.75rem] bg-cream-400 dark:bg-neutral-900 rounded-[2rem] md:rounded-[5rem] overflow-hidden'>
    ...
    <div className='mt-6'>
      {data?.Title && (
        <h1 className='font-bold font-heading text-heading-lg-sm lg:text-heading-lg text-neutral-900 dark:text-neutral-0'>
          {data?.Title}
        </h1>
      )}
      {data?.Description && (
        <p className='mt-6 text-body-lg-sm lg:text-body-lg text-neutral-900 dark:text-neutral-0'>{data?.Description}</p>
      )}
      {data?.ListTitle && (
        <h2 className='mt-12 font-bold font-heading text-subtitle text-aqua-500'>{data?.ListTitle}</h2>
      )}
