...
import Card from '@/app/components/Common/TrustCenter/Card-Center/Card';
...
const CardList = ({ data }: Readonly<Props>) => (
  <InnerContainer className='mt-[4.5rem]'>
    <div className='flex flex-col w-full gap-4 lg:flex-row lg:justify-space-between'>
      {Array.isArray(data) && data?.length > 0 && data?.map((content: any) => <Card card={content} key={content.id} />)}
    </div>
  </InnerContainer>
);
