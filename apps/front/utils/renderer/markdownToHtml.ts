...
renderer.link = function (href, title, text) {
  ...
  var linkIsUserMention =
    title && title.includes('s Profile - Hashnode') && href && href.includes('https://hashnode.com');
...
export const markdownToHtml = (contentMarkdown: string) => {
  const content = pipe(getHTMLFromMarkdown, getSanitizedHTML, extractMentions, getOptimizedImages)(contentMarkdown);
