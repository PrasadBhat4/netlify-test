...
import { generateId } from './ide-helper';
...
export async function getBootstrapData() {
  ...
  if (phCookie) {
    ...
    const fallbackCookie = cookieStore.get('cr_distinct_id');
    if (fallbackCookie) {
      distinct_id = fallbackCookie.value;
    } else {
      distinct_id = generateId();
    }
  }

  const client = new PostHog(phProjectAPIKey, { host: process.env.NEXT_PUBLIC_POSTHOG_HOST });
